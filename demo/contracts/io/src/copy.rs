...
use ckb_rust_std::io::{copy, repeat, sink, BufReader, BufWriter, Cursor, Read, Write};
use ckb_rust_std::io::error::Result;
use ckb_std::debug;
...
fn copy_specializes_bufwriter() {
    debug!("copy_specializes_bufwriter");
    let cap = 117 * 1024;
    let buf_sz = 16 * 1024;
...
fn copy_specializes_bufreader() {
    debug!("copy_specializes_bufreader");
...
fn copy_specializes_from_vecdeque() {
    debug!("copy_specializes_from_vecdeque");
...
fn copy_specializes_from_slice() {
    debug!("copy_specializes_from_slice");
