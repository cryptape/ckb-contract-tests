...
use ckb_rust_std::io::{error, repeat, BorrowedBuf, Cursor, SeekFrom};
use ckb_rust_std::io::{BufRead, BufReader, Read, Seek, Write};
use ckb_std::debug;
...
fn read_exact() {
    ...
    let mut c = Cursor::new(&b""[..]);
    assert_eq!(c.read_exact(&mut buf).unwrap_err().kind(), error::ErrorKind::UnexpectedEof);
...
fn read_buf_exact() {
    ...
    let mut c = Cursor::new(&b""[..]);
    assert_eq!(c.read_buf_exact(buf.unfilled()).unwrap_err().kind(), error::ErrorKind::UnexpectedEof);
...
fn main() ->i8 {
    ...
    lines();
    debug!("buf_read_has_data_left ");
    buf_read_has_data_left();

    //todo check
    // [contract debug] panicked at alloc/src/alloc.rs:441:13:
    ...
    debug!("bufreader_size_hint");
    // todo check 
    // panicked at contracts/io/src/io_all.rs:289:5:
